...
def ss97_loglike(tree,r,mu): #this implements the likelihood function of solow and smith 1997
    tree_like = [] 
    for i in tree.iternodes(order=1):
        if i == tree: 
            continue
        f = max(i.occurrences) * 10000
        l = min(i.occurrences)*10000
        tf = i.parent.height * 10000 #scaling factor is arbitrary
        tl = i.height * 10000
        a = math.log(mu*(math.pow(r,i.num_occurrences)))
        b = -(r+mu)*(f-l)
        c = math.log(r+mu)
        top = a+b
        cur_like = top-c
        tree_like.append(cur_like)
    return sum(tree_like)
